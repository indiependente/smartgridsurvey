...
\begin{quote}
\textit{``Una Smart Grid utilizza la tecnologia digitale per migliorare l'affidabilità, la sicurezza e l'efficienza (sia economica che energica) del sistema elettrico, ...RIVEDERE TRADUZIONE ... "}
\end{quote}
[4 ref] suggeriscono i seguenti attributi per una Smart Grid:
\begin{itemize}
\item Permette la gestione ``demand side" sia attraverso l'integrazione di smart meters, elettrodomestici intelligenti, micro-generazione e conservazione dell'energia sia fornendo agli utenti informazioni circa i loro utilizzi e i prezzi
...
\newline \newline
\textit{Quality of Service:} è necessario garantire la QoS per le teconologie di comunicazione e di rete utilizzate all'interno della Smart Grid, partendo dalla generazione dell'energia, passando per trasmissione e distribuzione e finendo alle applicazioni utente. In particolare, ci si focalizza su due parametri in particolare: latenza e larghezza di banda.\newline
...
\newline \newline
\textit{Interoperabilità:} si riferisce all'abilità delle diverse componenti di una Smart Grid di lavorare insieme, di utilizzare componenti compatibili, di scambiarsi informazioni tra di loro e lavorare in maniera cooperativa per completare i task. Tale requisito rende effettiva l'integrazione e la comunicazione bidirezionale tra i tanti elementi interconnessi della rete.\newline
Il NIST ha sviluppato un framework che include protocolli e standard per la gestione delle informazioni, in modo da raggiungere l'interoperabilità tra i dispositivi della Smart Grid e i sistemi. Propone un approccio per identificare gli standard costituito da tre fasi: nella prima, ci si occupa di trovare degli stakeholder che partecipino ad un processo pubblico di analisi delle problematiche degli attuali standard e di definizione di priorità per i nuovi standard. Tale fase termina con la pubblicazione di un report. Nella seconda fase si stabilisce una partnership tra pubblico e privato e si forma un pannello di interoperabilità per guidare il progresso a lungo termine. Nella terza ed ultima fase si sviluppa un framework per il testing e la certificazione di come gli standard sono implementati nei dispositivi, nei processi e nei sistemi.
\newline \newline
\textit{Scalabilità:}
\newpage
