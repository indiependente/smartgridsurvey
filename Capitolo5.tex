...
\subsection{OMA Key}
Durante il processo di produzione industriale, i dispositivi OSGP vengono configurati con una \textit{Open Media Access Key} (OMAK) univoca a 96 bit (maggiori dettagli in \ref{subsec:OSGPencr}). Tali chiavi sono poi trasmesse in maniera sicura alle società di servizi, in modo da poter essere utilizzate poi dai data concentrator.\newline Tipicamente, l'originaria OMAK specifica per ogni dispositivo, viene poi sostituita dal data concentrator con una OMAK condivisa.\newline
L'OMAK è utilizzata per i seguenti scopi:
\begin{itemize}
